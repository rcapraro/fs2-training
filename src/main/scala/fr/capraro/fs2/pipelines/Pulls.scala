...
object Pulls extends IOApp.Simple {
  ...
  override def run: IO[Unit] = {
    ...
    val donePull: Pull[Pure, Nothing, Unit] = Pull.done // signal end of the pull
    ...
    // turn a Stream into a Pull
    val toPull: ToPull[Pure, Int]                            = s.pull
    val echoPull: Pull[Pure, Int, Unit]                      = s.pull.echo
    val takePull: Pull[Pure, Int, Option[Stream[Pure, Int]]] = s.pull.take(3) // Option represents the rest of the Stream
    val dropPull: Pull[Pure, Int, Option[Stream[Pure, Int]]] = s.pull.drop(3)

    // Exo -> implements using pulls
    ...
    def runningMax: Pipe[Pure, Int, Int] = s => {
      s.scanChunksOpt(Int.MinValue) { maxAcc =>
        Some { (chunk: Chunk[Int]) =>
          val newMax = chunk.foldLeft(Int.MinValue)(_ max _) max maxAcc
