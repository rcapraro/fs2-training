...
object Join extends IOApp.Simple {
  ...
  override def run: IO[Unit] = {
    ...
    val s3Infinite = Stream.iterate(3_000_000)(_ + 1).covary[IO].metered(50.millis)
    val s4Infinite = Stream.iterate(4_000_000)(_ + 1).covary[IO].metered(50.millis)
    ...
    val s1Infinite = Stream.iterate(1_000_000)(_ + 1).covary[IO].metered(50.millis)
    ...
    // Run the producer and the consumers in parallel
    // Finish after 5 seconds
    Stream.eval(Queue.unbounded[IO, Int]).flatMap { queue =>
      val ps: Stream[Pure, Stream[IO, Nothing]]  = Stream.range(0, 5).map(id => producer(id, queue))
      val cs: Stream[Pure, Stream[IO, Nothing]]  = Stream.range(0, 10).map(id => consumer(id, queue))
      val all: Stream[Pure, Stream[IO, Nothing]] = ps ++ cs
